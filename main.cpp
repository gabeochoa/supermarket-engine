...
bool inBounds(vec2f pos) {
  return (pos.x <= PWIDTH) && (pos.x >= 0) && (pos.y < PHEIGHT) && (pos.y >= 0);
}
  ...
  while (!openSet.empty()) {
    ...
    for (int i = 0; i < 8; i++) {
      ...
      if (tentative_gScore < gScore[neighbor].value) {
        ...
        if (it == openSet.end()) {
          openSet.push_back(QueueItem{fScore[neighbor].value, neighbor});
        }
      }
    }
  }
...
struct Customer : Entity {
  ...
  void update(sf::Time elapsed) {
    ...
    while (desireIndex < desires.size()) {
      ...
      // TODO check if store even has the item before looping through shelves
      auto matchingShelf = shelf_with_item(want);
      if (matchingShelf != shelves.end()) {
        auto shelf_pos = (*matchingShelf)->colliders[0]->getPosition();
        // so you cant place a spot on the shelf, so we need to go right on the
        // side of it
    ...
    if (desireIndex >= desires.size()) {
      // TODO - handle the case where customer cant find their remaining items
      idle = true;
      return;
    }
    ...
    for (int i = 0; i < entities.size(); i++) {
      ...
      if (typeid(*entity) == typeid(Customer)) {
        ...
        for (desire d : cust->desires) {
          ...
          auto it = std::find_if(desires.begin(), desires.end(),
                                 [&](const desire &des) {
...
struct HUD : public sf::Drawable {
  ...
  HUD() {
    ...
    uitext.insert(strUI("money", new UIText(H_PADDING, V_PADDING + V_SPACING)));
    uitext.insert(strUI("idle_shoppers",
                        new UIText(H_PADDING, V_PADDING + (V_SPACING * 2))));
    uitext.insert(strUI("top_desired",
                        new UIText(H_PADDING, V_PADDING + (V_SPACING * 3))));
  }
