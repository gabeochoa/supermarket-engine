...
Path bfs(const glm::vec2& start, const glm::vec2& end) {
    ...
    while (!q.empty()) {
        ...
        glm::vec2 pos = p.back();
        if (pos == end) return p;
        if (visited[pos.y * MAP_W + pos.x]) continue;
...
Path pathfind(const glm::vec2& start, const glm::vec2& end) {
    Theta t(start, end,
            [](auto pos) { return inBound(pos) && !isColor(pos, GRAY); });
    t.options.disableLineOfSight = true;
    return t.go();
...
struct DemoLayer : public Layer {
    ...
    DemoLayer() : Layer("") {
        ...
        {
            // Choose an example start and end
            // and force pathdinfind to run
            start = glm::vec2{3, 5};
            setTileColor(start, BLUE);
            end = glm::vec2{MAP_W - 5, MAP_H - 8};
            setTileColor(end, GREEN);
    ...
    void init_map() {
        for (int i = 0; i < MAP_W; i++) {
            for (int j = 0; j < MAP_H; j++) {
                ...
                // Place a bunch of walls in the middle of the map
                // with a single hole
                if (i == floor(MAP_W / 2) && j != 0 && j != MAP_H - 1 &&
                    j != floor(MAP_H / 2)) {
                    setTileColor(glm::vec2{i, j}, GRAY);
                }
            }
        }
    }
    ...
    void budgeEnd(Time dt) {
        moveTimer += dt.s();
        if (moveTimer >= 0.1f) {
            moveTimer = 0.f;
            setTileColor(end, BLACK);

            int i = 0;
            do {
                float amt = rand() % 2 == 0 ? 1 : -1;
                if (rand() % 2 == 0)
                    end.x += amt;
                else
                    end.y += amt;
    ...
    bool onKeyPressed(KeyPressedEvent& event) {
        ...
        if (event.keycode == Key::KeyCode::D3) selectedTool = 3;
